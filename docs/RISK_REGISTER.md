...
|----|----------|------------------|------------------|------------|----------------|-----------------------------------------------|--------|
| R-VIB-001 | Technical | Vibration-induced beam wander and alignment drift on propeller-driven or VTOL platforms decenter the multi-point pattern on target sensor FOV | Engagement miss; effective irradiance on sensor reduced by 50–100% (order of magnitude) at moderate wander angles | M | First principles (angular jitter × range → linear displacement); no flight data | Elastomer isolation (low SWaP); shorten range; widen pattern spacing (reduces peak I); active stabilization (high SWaP/cost — rejected Phase 0) | Open |
...
| R-EFF-001 | Technical | Limited validated open data on real-world effectiveness against military or hardened FPV UAS sensors (filters, AGC, burst modes, AI rejection) | System may not degrade target sensor despite meeting bench surrogate tests | H | Sparse public flight-test data; vendor marketing not evidence | Higher power (SWaP/eye safety conflict); multi-wavelength (cost/complexity); accept mission limitation | Open |
